    ...
    restrictedStockUnits: [
      {
        ...
        vestingSchedules: [
          { vestDate: "2024-09-01T00:00:00", vestAmount: 250 },
        ],
      },
    ],
  },
...
describe("canImport", () => {
  it("accepts a JSON file with awardsDetail.restrictedStockUnits", () => {
    expect(
      schwab.canImport("Awards.json", JSON.stringify(minimalAward)),
    ).toBe(true);
  });

  it("rejects non-JSON filenames", () => {
    expect(
      schwab.canImport("data.csv", JSON.stringify(minimalAward)),
    ).toBe(false);
  });
});
...
describe("import", () => {
  it("parses grants with past and future vestings", () => {
    const grants = schwab.import(
      "Awards.json",
      JSON.stringify(minimalAward),
    );
    ...
    expect(g.awardDate.equals(Temporal.PlainDate.from("2023-06-15"))).toBe(
      true,
    );
    expect(g.vests).toHaveLength(3);
    expect(g.vests[0].shares).toBe(250);
    expect(g.vests[0].date.equals(Temporal.PlainDate.from("2024-03-01"))).toBe(
      true,
    );
  });
});
